...
--
-- Regroupe les migrations qui n'ont JAMAIS pu s'appliquer (chacune échouait
-- sur une erreur SQL), plus le correctif du trigger de flocage qui bloquait
-- toute création de commande :
--   0012 — vues rapports        (i.sales_order_id / ef.assigned_to inexistants)
--   0027 — numérotation         (generate_claim_number : changement de type)
--   0028 — messagerie, stock,
--          objectifs, SMS        (suppliers déjà créée + 3 policies invalides)
--   0029b— couleurs/tailles      (dépendait de 0028)
--   0031 — variantes produits    (seed sans sku, colonne NOT NULL)
--   0032 — paiements + flocage   (policy FOR INSERT ... USING invalide)
--
-- Vérifié sur PostgreSQL 16 : reconstruction complète des 33 migrations sans
-- erreur, puis application de ce lot sur la base migrée, sans erreur.
-- ============================================================================
